...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, required=True, choices=["GazeinTheWild", "Dikablis", "LPW"], help="Name of the dataset.")
    ...
    video_paths = sorted(glob.glob(os.path.join(args.data_dir, dataset_name, "VIDEOS", "*.mp4")))
    print(f"Found {len(video_paths)} videos in dataset {dataset_name}.")
    ...
    for vid, video_path in enumerate(video_paths):
        ...
        try:
            ...
            if num_frames != annotation_df.shape[0]:
                print(f"Warning: Number of frames in video ({num_frames}) and annotation ({annotation_df.shape[0]}) do not match for video {video_path}!")
                continue
            ...
            if os.path.exists(out_video_dir) and os.path.exists(out_iris_dir) and os.path.exists(out_annotation_path):
                print(f"Output for video {annotation_name} already exists, skipping...")
                continue
            ...
            print(f"Error processing video {annotation_name}: {e}")
            continue